...
method update_when_needed (
    ...
    my @provisioning_obligations = $self
        ->update_smart(
            ...
            update_include => {
                fields_match => [ qw(provisioning_agreement_id service_type_id service_level_id) ],
            },
            update_exclude => {
                conditions_match => [ { id => { -in => $excluded_ids } } ]
            },
            now => $now
        );
    ...
    if(scalar(@provisioning_obligations) > 1) {
        $self->get_logger->warnf(
            "Multiple provisioning obligations found, expected only one: %s",
