...
foreach my $provider ($schema->resultset('Contractor')->search({ provider => 1 })) {
    printf("Provider: %s %s\n", $provider->search_related('contractor_type')->first->search_related('contractor_type_i18ns', { language_id => 2 })->first->name_short, $provider->name);
    foreach my $service_agreement ($schema->resultset('ServiceAgreement')->search({ provider_contractor_id => $provider->id })) {
        ...
        printf(" ` Contract: %s - %s %s\n", $service_agreement->name, $client->search_related('contractor_type')->first->search_related('contractor_type_i18ns', { language_id => 2 })->first->name_short, $client->name);
        foreach my $service_obligation ($schema->resultset('ServiceObligation')->search({ service_agreement_id => $service_agreement->id })) {
            my $service = $service_obligation->search_related('service_type')->first;
            printf(" ` ` Service: %s - %s\n", $service->id, $service->search_related('service_i18ns', { language_id => 2 })->first->name);
        }
    }
}
